"""crypto_bot/league/league_status.py — Trading Bot League heartbeat adapter.
...
Safety properties:
  ...
  * Lazy env reads. Nothing here calls os.getenv() at import time, so it's
    safe to import before main.py runs load_dotenv().
  * No new dependencies. Uses `requests` which is already a transitive
    dependency of the crypto bot via supabase / public api clients.
...
try:
    import requests
except ImportError:  # pragma: no cover - bot already has requests transitively
...
def end_run(
    ...
    *,
    ...
    Status mapping for callers using non-League vocabulary:
      crypto bot's "completed" → pass status="success"
      crypto bot's "error"     → pass status="failed"
    """
    global _current_run_id
    cfg = _config()
...
def log_trade(
    ...
    *,
    ...
    Asset class is always 'crypto' for this bot. The unique partial index
    on (bot_id, order_id) in the table makes repeat inserts of the same
    Public order_id idempotent at the DB level. One shot per call.
    """
    cfg = _config()
    if cfg is None:
        return
    ...
    if side_u not in ("BUY", "SELL", "SHORT", "COVER"):
        ...
        return
    rid = run_id if run_id is not None else _current_run_id
    # Auto-compute amount_usd if both quantity and price are present and
    # the caller didn't pass one explicitly — matches the crypto bot's
    # convention where supabase_logger.log_trade takes price + size.
    if amount_usd is None and quantity is not None and price is not None:
        try:
